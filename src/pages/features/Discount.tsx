      ...
      <div className="flex flex-col h-full lg:flex-row gap-2 max-w-5xl mx-auto w-full min-h-0 overflow-y-auto">
        ...
        <div className="flex-1 flex flex-col bg-purple-50 rounded-2xl p-3 border-2 border-purple-200 shrink-0">
          ...
          <div className="grid grid-cols-3 gap-2 flex-grow min-h-[160px]">
            ...
            <button onClick={clearNum} className="bg-rose-100 border-b-4 border-rose-300 rounded-xl text-xl font-black text-rose-700 hover:bg-rose-200 active:border-b-0 active:translate-y-1 transition-all shadow-sm">清除</button>
        ...
        <div className="flex-1 flex flex-col gap-2 shrink-0">
          <div className="bg-purple-50 rounded-2xl p-3 border-2 border-purple-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => applyDiscount(0.9, '9折')} className={`p-3 rounded-xl text-xl font-black border-b-4 active:border-b-0 active:translate-y-1 transition-all shadow-sm ${discount === 0.9 ? 'bg-purple-600 border-purple-800 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}>9折</button>
              <button onClick={() => applyDiscount(0.8, '8折')} className={`p-3 rounded-xl text-xl font-black border-b-4 active:border-b-0 active:translate-y-1 transition-all shadow-sm ${discount === 0.8 ? 'bg-purple-600 border-purple-800 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}>8折</button>
              <button onClick={() => applyDiscount(0.7, '7折')} className={`p-3 rounded-xl text-xl font-black border-b-4 active:border-b-0 active:translate-y-1 transition-all shadow-sm ${discount === 0.7 ? 'bg-purple-600 border-purple-800 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}>7折</button>
              <button onClick={() => applyDiscount(0.5, '半價')} className={`p-3 rounded-xl text-xl font-black border-b-4 active:border-b-0 active:translate-y-1 transition-all shadow-sm ${discount === 0.5 ? 'bg-red-500 border-red-700 text-white' : 'bg-white border-slate-300 text-red-600 hover:bg-slate-50'}`}>半價(5折)</button>
            </div>
          </div>
